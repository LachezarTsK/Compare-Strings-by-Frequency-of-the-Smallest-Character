...
function numSmallerByFrequency(queries: string[], words: string[]): number[] {
    const frequenciesOfLexicographicallySmallestChars: number[] = new Array(words.length);
...
function findFrequencyOfLexicographicallySmallestChar(word: string): number {
...
function binarySearchForInsertionIndex(target: number, input: number[]): number {
    let left = 0;
